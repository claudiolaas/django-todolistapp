...
def currenttodos(request):
    todos = Todo.objects.filter(user=request.user, complete_date__isnull=True)   
    return render(request, 'todo/currenttodos.html', {'todos':todos})

@login_required 
def completedtodos(request):
    todos = Todo.objects.filter(user=request.user, complete_date__isnull=False).order_by('-complete_date')   
